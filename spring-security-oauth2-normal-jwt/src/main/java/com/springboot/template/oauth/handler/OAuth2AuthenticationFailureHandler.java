package com.springboot.template.oauth.handler;
...
import static com.springboot.template.oauth.repository.OAuth2AuthorizationRequestBasedOnCookieRepository.REDIRECT_URI_PARAM_COOKIE_NAME;

/**
 * OAuth2 인증 과정에서 인증 실패 시 처리를 담당하는 클래스.
 * OAuth2 인증 과정에서 인증 과정이 실패했을 때, 이 클래스는 클라이언트가 인증 요청을 보낸 리다이렉트 URL의 쿠키에서
 * `REDIRECT_URI_PARAM_COOKIE_NAME` 이름으로 저장된 값을 가져와서, 그 값으로 리다이렉트한다.
 * 이 때, 리다이렉트 URL에 error 쿼리 파라미터를 추가하고,
 * 해당 파라미터 값으로는 exception 객체의 getLocalizedMessage() 메소드로 얻을 수 있는 인증 실패 메시지를 넣는다.
